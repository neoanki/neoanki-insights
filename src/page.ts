...
root.innerHTML = `<main aria-busy="true"><p>Read-only extension</p><h2>Collection insights</h2><p>Current workload and organization facts, not mastery scores.</p><button class="refresh" type="button" disabled>Refresh</button><p class="status" role="status" aria-live="polite">Reading the collection…</p><div class="content"></div></main>`
...
void createSandboxedUiClient().then(async (client) => {
  ...
  const load = async () => {
    refresh.disabled = true; status.textContent = 'Reading the collection…'
    try { const result = await client.call<{ summary: CollectionInsights }>('command', { commandId: 'insights.refresh' }); render(result.summary, hostSummary.summary?.dueToday); status.textContent = 'Insights refreshed.' }
    catch (error) { status.textContent = error instanceof Error ? error.message : 'Insights could not be loaded.' }
    finally { refresh.disabled = false; root.querySelector('main')?.setAttribute('aria-busy', 'false') }
  }
  refresh.onclick = () => void load()
  await load()
}).catch((error) => { root.querySelector('main')?.setAttribute('aria-busy', 'false'); status.textContent = error instanceof Error ? error.message : 'Insights are unavailable.' })
